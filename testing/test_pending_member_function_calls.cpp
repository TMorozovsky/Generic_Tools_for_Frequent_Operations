...
namespace
{
    ...
    using test_holder = polymorphic_holder<Base, 16, 8, polymorphic_holder_MCP::no_moving_or_copying, polymorphic_holder_DRP::bytes_array_only>;
    ...
    void run_test()
    {
        ...
        ASSERT_EQ(result_pmf_simple               , 11);
        ASSERT_EQ(result_pmf_const                , 12);
        ASSERT_EQ(result_pmf_volatile             , 13);
        ASSERT_EQ(result_pmf_const_volatile       , 14);

        ASSERT_EQ(result_pmf_simple_lvalue        , 15);
        ASSERT_EQ(result_pmf_const_lvalue         , 16);
        ASSERT_EQ(result_pmf_volatile_lvalue      , 17);
        ASSERT_EQ(result_pmf_const_volatile_lvalue, 18);
