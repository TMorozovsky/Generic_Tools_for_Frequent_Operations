...
{
    // Tells whether type T is an unsigned integral type which can be used by
    // polymorphic_holder's Data Representation Policies to store the offset to base subobject.
...
namespace polymorphic_holder_utils
{
    ...
    // and implemented in each derived class via using the GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_ONLY macro
    ...
    // Interface that must be inherited by the base class of a polymorphic hierarchy
    // and implemented in each derived class via using the GTFO_POLYMORPHIC_HOLDER_THROWING_MOVEABLE_ONLY macro
    ...
    // Interface that must be inherited by the base class of a polymorphic hierarchy
    // and implemented in each derived class via using the GTFO_POLYMORPHIC_HOLDER_NOTHROW_COPYABLE_ONLY macro
    ...
    // Interface that must be inherited by the base class of a polymorphic hierarchy
    // and implemented in each derived class via using the GTFO_POLYMORPHIC_HOLDER_THROWING_COPYABLE_ONLY macro
    ...
    // Interface that must be inherited by the base class of a polymorphic hierarchy
    // and implemented in each derived class via using the GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_AND_NOTHROW_COPYABLE macro
    ...
    // Interface that must be inherited by the base class of a polymorphic hierarchy
    // and implemented in each derived class via using the GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_AND_THROWING_COPYABLE macro
    ...
    // Interface that must be inherited by the base class of a polymorphic hierarchy
    // and implemented in each derived class via using the GTFO_POLYMORPHIC_HOLDER_THROWING_MOVEABLE_AND_THROWING_COPYABLE macro
...
#define GTFO_POLYMORPHIC_HOLDER_OVERRIDE_NOTHROW_MOVE_CONSTRUCT_AT(c) \
    ...
    { \
        ...
        ::new (dest_buffer_begin) c(::gtfo::move(*this)); \
    }

// Helper macro that can be used inside a class definition
// in order to provide a typical implementation of the throwing move_construct_at() virtual function.
#define GTFO_POLYMORPHIC_HOLDER_OVERRIDE_THROWING_MOVE_CONSTRUCT_AT(c) \
...
#define GTFO_POLYMORPHIC_HOLDER_OVERRIDE_NOTHROW_COPY_CONSTRUCT_AT(c) \
...
#define GTFO_POLYMORPHIC_HOLDER_OVERRIDE_THROWING_COPY_CONSTRUCT_AT(c) \
...
#define GTFO_POLYMORPHIC_HOLDER_DELETE_COPY_CONSTRUCT_AT() \
    void copy_construct_at(unsigned char *, size_t) const = delete;

// Helper macro that can be used inside a class definition in order to delete the move_construct_at() function.
#define GTFO_POLYMORPHIC_HOLDER_DELETE_MOVE_CONSTRUCT_AT() \
    void move_construct_at(unsigned char *, size_t) && = delete;
...
#define GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_ONLY(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_NOTHROW_MOVE_CONSTRUCT_AT(c) \
    GTFO_POLYMORPHIC_HOLDER_DELETE_COPY_CONSTRUCT_AT()
...
#define GTFO_POLYMORPHIC_HOLDER_THROWING_MOVEABLE_ONLY(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_THROWING_MOVE_CONSTRUCT_AT(c) \
    GTFO_POLYMORPHIC_HOLDER_DELETE_COPY_CONSTRUCT_AT()
...
#define GTFO_POLYMORPHIC_HOLDER_NOTHROW_COPYABLE_ONLY(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_NOTHROW_COPY_CONSTRUCT_AT(c) \
    GTFO_POLYMORPHIC_HOLDER_DELETE_MOVE_CONSTRUCT_AT()
...
#define GTFO_POLYMORPHIC_HOLDER_THROWING_COPYABLE_ONLY(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_THROWING_COPY_CONSTRUCT_AT(c) \
    GTFO_POLYMORPHIC_HOLDER_DELETE_MOVE_CONSTRUCT_AT()
...
#define GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_AND_NOTHROW_COPYABLE(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_NOTHROW_MOVE_CONSTRUCT_AT(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_NOTHROW_COPY_CONSTRUCT_AT(c)
...
#define GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_AND_THROWING_COPYABLE(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_NOTHROW_MOVE_CONSTRUCT_AT(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_THROWING_COPY_CONSTRUCT_AT(c)
...
#define GTFO_POLYMORPHIC_HOLDER_THROWING_MOVEABLE_AND_THROWING_COPYABLE(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_THROWING_MOVE_CONSTRUCT_AT(c) \
    GTFO_POLYMORPHIC_HOLDER_OVERRIDE_THROWING_COPY_CONSTRUCT_AT(c)
...
namespace polymorphic_holder_lib
{
    ...
    inline void copy_as_bytes(BitwiseCopyablePolymorphicHolder & dest, const BitwiseCopyablePolymorphicHolder & src) noexcept
    {
        ...
        ::gtfo::memcpy_s(static_cast<void *>(&dest), sizeof(dest), static_cast<const void *>(&src), sizeof(src));
    }
    ...
    inline void set_bytes_to_zero(PolymorphicHolder & dest) noexcept
    {
        ...
        ::gtfo::memset(static_cast<void *>(&dest), 0, sizeof(dest));
    }
    ...
    public:
        constexpr explicit scoped_throwing_construction_guard(PolymorphicHolder & ref_polymorphic_holder) noexcept
...
namespace polymorphic_holder_MCP
{
    ...
    // and
    //     - adding the GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_ONLY(DerivedType) macro
    ...
    protected:
        ...
        inline void MCP_move_construct_from(PolymorphicHolderSelf && other) noexcept
        {
            polymorphic_holder_lib::nothrow_move_construct(static_cast<PolymorphicHolderSelf &>(*this), ::gtfo::move(other));
        }
    ...
    // and
    //     - adding the GTFO_POLYMORPHIC_HOLDER_THROWING_MOVEABLE_ONLY(DerivedType) macro
    //       to all of derived classes' definitions.
    // Does not allow copying.
    template<class PolymorphicHolderSelf>
    ...
    protected:
        ...
        {
            polymorphic_holder_lib::throwing_move_construct(static_cast<PolymorphicHolderSelf &>(*this), ::gtfo::move(other));
        }
    ...
    // and
    //     - adding the GTFO_POLYMORPHIC_HOLDER_NOTHROW_COPYABLE_ONLY(DerivedType) macro
    //       to all of derived classes' definitions.
    ...
    // and
    //     - adding the GTFO_POLYMORPHIC_HOLDER_THROWING_COPYABLE_ONLY(DerivedType) macro
    //       to all of derived classes' definitions.
    // Moving is the same operation as copying.
    template<class PolymorphicHolderSelf>
    ...
    // and
    //     - adding the GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_AND_NOTHROW_COPYABLE(DerivedType) macro
    //       to all of derived classes' definitions.
    template<class PolymorphicHolderSelf>
    ...
    // and
    //     - adding the GTFO_POLYMORPHIC_HOLDER_NOTHROW_MOVEABLE_AND_THROWING_COPYABLE(DerivedType) macro
    //       to all of derived classes' definitions.
    template<class PolymorphicHolderSelf>
    ...
    // and
    //     - adding the GTFO_POLYMORPHIC_HOLDER_THROWING_MOVEABLE_AND_THROWING_COPYABLE(DerivedType) macro
    //       to all of derived classes' definitions.
    template<class PolymorphicHolderSelf>
...
{
    static_assert(::gtfo::_tt::is_polymorphic<BaseType>::value, "base type must be polymorphic");
...
public:
        ...
        , drp_type()
    {
        ...
        this->MCP_move_construct_from(::gtfo::move(other));
    }
    ...
    {
        GTFO_DEBUG_ASSERT(this->owns_object());
    ...
    {
        GTFO_DEBUG_ASSERT(this->owns_object());
        GTFO_DEBUG_ASSERT(p_member_field != nullptr);
    ...
    inline void construct(CtorArgs&&... args) noexcept(noexcept(DesiredType(::gtfo::forward<CtorArgs>(args)...)))
    {
        GTFO_DEBUG_ASSERT(!this->owns_object());
        constexpr bool construction_nothrow_flag = noexcept(DesiredType(::gtfo::forward<CtorArgs>(args)...));
        ...
        this->impl_construct<DesiredType>(construction_specification_tag(), ::gtfo::forward<CtorArgs>(args)...);
    }
    ...
    template<class DesiredType, typename... CtorArgs>
    inline void reset(CtorArgs&&... args) noexcept(noexcept(DesiredType(::gtfo::forward<CtorArgs>(args)...)))
    {
        constexpr bool construction_nothrow_flag = noexcept(DesiredType(::gtfo::forward<CtorArgs>(args)...));
        using construction_specification_tag = typename polymorphic_holder_lib::get_noexcept_specification_tag<construction_nothrow_flag>::result;
        ...
        this->impl_reset<DesiredType>(construction_specification_tag(), move_specification_tag(), ::gtfo::forward<CtorArgs>(args)...);
    }
    ...
    // polymorphic_holder's MaxObjectSize and ObjectAlignment.
    template<class DesiredType, typename... CtorArgs>
    inline static polymorphic_holder make(CtorArgs&&... args) noexcept(noexcept(DesiredType(::gtfo::forward<CtorArgs>(args)...)) && mcp_type::MCP_is_nothrow_move_constructible)
    {
        polymorphic_holder temp;
        temp.construct<DesiredType>(::gtfo::forward<CtorArgs>(args)...);
        return temp;
    }
...
private: // impl_copy_assign(const polymorphic_holder & other, copy_construction_specification_tag, move_assignment_specification_tag)
    ...
    inline void impl_copy_assign(const polymorphic_holder & other, polymorphic_holder_lib::throwing_tag, polymorphic_holder_lib::nothrow_tag)
    {
        ...
        static_assert(noexcept(*this = ::gtfo::move(temp)), "");
        *this = ::gtfo::move(temp);
    }
...
private: // impl_reset<DesiredType>(construction_specification_tag, move_specification_tag, args)
    ...
    inline void impl_reset(polymorphic_holder_lib::nothrow_tag, move_specification_tag, CtorArgs&&... args) noexcept
    {
        static_assert(noexcept(DesiredType(::gtfo::forward<CtorArgs>(args)...)), "");
        this->destroy_object_safe();
        this->impl_construct<DesiredType>(polymorphic_holder_lib::nothrow_tag(), ::gtfo::forward<CtorArgs>(args)...);
    }
    ...
    inline void impl_reset(polymorphic_holder_lib::throwing_tag, polymorphic_holder_lib::nothrow_tag, CtorArgs&&... args)
    {
        ...
        static_assert(!noexcept(DesiredType(::gtfo::forward<CtorArgs>(args)...)), "");
        static_assert(noexcept(*this = ::gtfo::move(temp)), "");
        temp.impl_construct<DesiredType>(polymorphic_holder_lib::throwing_tag(), ::gtfo::forward<CtorArgs>(args)...);
        *this = ::gtfo::move(temp);
    }
    ...
    {
        this->destroy_object_safe();
        this->impl_construct<DesiredType>(polymorphic_holder_lib::throwing_tag(), ::gtfo::forward<CtorArgs>(args)...);
    }
};
